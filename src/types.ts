...
export interface PluralSifatMusyabihat {
  ...
  isQiyasi?: boolean;
  isSamai?: boolean;
  qiyasi_katsroh?: string;
  qiyasi_qillah?: string;
  qiyasi_muntahal?: string;
  mufrod_mudzakkar?: string;
  mufrod_muannas?: string;
  wazan_name?: string;
  // Optional Qiyasi Metadata
  qiyasi?: {
    keterangan: string;
    samai: boolean;
    wazan_qiyasi: {
    ...
    jama_qillah: string[];
    jama_kathrah: string[];
    jama_muntahal_jumu: string[];
    catatan_ilal: string;
    contoh: Array<{
      label: string; // e.g. "حَسِنٌ"
      singular: string; // instantiated singular
      qillah?: string; // instantiated qillah
      kathrah: string; // instantiated katsroh
      wazan: string; // e.g. "فَعِل → فِعَال"
    }>;
  };
}
