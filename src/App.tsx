...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<"istilahi" | "lughowi" | "jama" | "iilal" | "favorites">("istilahi");
...
module.exports = config;`
  ...
  // AI Morphological explanation via Gemini Core
  const triggerAiExplanation = async () => {
    setAiExplainLoading(true);
    setAiExplanationText("");
    
    const promptParams = `fa=${selectedEntry.root.fa}&ain=${selectedEntry.root.ain}&lam=${selectedEntry.root.lam}&babNum=${selectedEntry.babNum}&bina=${selectedEntry.bina || "Shohih"}&translation=${selectedEntry.translation}`;
    
    try {
      logTerminal(`[Pembaca AI] Meminta analisis morfologi shorof tuntas lewat Gemini AI untuk kata '${selectedEntry.root.fa}${selectedEntry.root.ain}${selectedEntry.root.lam}'...`);
      const response = await fetch(`/api/gemini/explain?${promptParams}`);
      const data = await response.json();
      
      if (data.success) {
        setAiExplanationText(data.explanation);
        logTerminal(`[Pembaca AI] Berhasil menerima penjelasan Gemini AI.`);
      } else {
        throw new Error(data.explanation || "Gagal menghubungi API server");
      }
    } catch (err: any) {
...
*Analisis ini disajikan secara cepat dari database luring sistem.*`;
      setAiExplanationText(fallback);
      logTerminal(`[Asisten AI] Menggunakan fallback kamus luring dikarenakan: ${err.message}`);
    } finally {
                  ...
                  <div className="space-y-3 z-10 relative">
                    ...
                    {activeTab === "lughowi" && (
                      <div className="space-y-3">
                        <p className="text-[10px] text-slate-400 uppercase tracking-widest px-1 font-bold">14 Konjugasi Pronoun (Lughowi)</p>
                        
                        {/* Render Compact Lughowi Tables */}
                        <div className="max-h-[300px] overflow-y-auto space-y-2 pr-1 text-xs">
                          {[
                            { label: "هُوَ (Dia Lk Lk)", val: calculatedTasrif.madhi },
                            { label: "هُمَا (Mereka 2 Lk Lk)", val: calculatedTasrif.madhi + "ا" },
                            { label: "هُمْ (Mereka Jamak Lk Lk)", val: calculatedTasrif.madhi + "ُوْا" },
                            { label: "هِيَ (Dia Pr Pr)", val: calculatedTasrif.madhi + "َتْ" },
                            { label: "هُمَا (Mereka 2 Pr Pr)", val: calculatedTasrif.madhi + "َتَا" },
                            { label: "هُنَّ (Mereka Jamak Pr)", val: calculatedTasrif.madhi.slice(0, -1) + "ْنَ" },
                            { label: "أَنْتَ (Kamu Lk Lk)", val: calculatedTasrif.madhi.slice(0, -1) + "ْتَ" },
                            { label: "أَنْتُمَا (Kalian 2)", val: calculatedTasrif.madhi.slice(0, -1) + "ْتُمَا" },
                            { label: "أَنْتُمْ (Kalian Jamak Lk Lk)", val: calculatedTasrif.madhi.slice(0, -1) + "ْتُمْ" },
                            { label: "أَنْتِ (Kamu Pr Pr)", val: calculatedTasrif.madhi.slice(0, -1) + "ْتِ" },
                            { label: "أَنَّكُنَّ (Kalian Jamak Pr)", val: calculatedTasrif.madhi.slice(0, -1) + "ْتُنَّ" },
                            { label: "أَنَا (Saya)", val: calculatedTasrif.madhi.slice(0, -1) + "ْتُ" },
                            { label: "نَحْنُ (Kami / Kita)", val: calculatedTasrif.madhi.slice(0, -1) + "ْنَا" }
                          ].map((lItem, lid) => (
                            <div key={lid} className="bg-slate-900 border border-slate-800 p-2 rounded-xl flex items-center justify-between">
                              <span className="text-[10px] text-slate-400">{lItem.label}</span>
                              <span className="font-arabic font-extrabold text-amber-400 select-all">{lItem.val}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                        ...
                        <div className="bg-slate-900 border border-slate-800/80 p-3 rounded-2xl space-y-3.5 text-xs">
                          <div className="space-y-1">
                            ...
                            <div className="flex items-center justify-between bg-slate-950 p-2 rounded-xl border border-slate-900">
                              <span className="font-mono text-[10px] text-slate-400">مُنْتَهَى الجُمُوعِ</span>
                          ...
                          <div className="space-y-1">
                            <span className="block text-[8px] uppercase tracking-wider text-slate-500">Jumlah Tunjang Paucity (Sedikit):</span>
                            <div className="flex items-center justify-between bg-slate-950 p-2 rounded-xl border border-slate-900">
                              <span className="font-mono text-[10px] text-slate-400">جَمْعُ قِلَّةٍ</span>
                              {activeJamakTab === "fail" && <span className="font-arabic font-extrabold text-indigo-400 text-base select-all">{structuralPlurals.fail.qillah || "—"}</span>}
                              {activeJamakTab === "maful" && <span className="font-arabic font-extrabold text-indigo-400 text-base select-all">{structuralPlurals.maful.qillah || "—"}</span>}
                              {activeJamakTab === "zamanmakan" && <span className="font-arabic font-extrabold text-indigo-400 text-base select-all">{structuralPlurals.zamanmakan.qillah || "—"}</span>}
                              {activeJamakTab === "alat" && <span className="font-arabic font-extrabold text-indigo-400 text-base select-all">{structuralPlurals.alat.qillah || "—"}</span>}
                              {activeJamakTab === "sifat" && <span className="font-arabic font-extrabold text-indigo-400 text-base select-all">{structuralPlurals.sifat.qillah || "—"}</span>}
                            </div>
                          </div>
                      ...
                      <div className="space-y-2.5">
                        <div className="flex items-center justify-between px-1">
                          ...
                          <button
                            ...
                            className="bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-800 text-white rounded-lg px-2 py-1 text-[10px] font-bold flex items-center space-x-1 transition select-none cursor-pointer"
                          >
                            <Sparkles className="w-3 h-3 animate-spin" />
                            <span>{aiExplainLoading ? "Memikir..." : "Ulas via AI"}</span>
                          </button>
                        </div>
                        ...
                        <div className="bg-slate-900 border border-slate-800 p-3.5 rounded-2xl space-y-2.5 max-h-[300px] overflow-y-auto pr-1">
                          {aiExplanationText ? (
                            ...
                            <div className="py-8 text-center text-slate-500 space-y-1 select-none">
                              ...
                              <p className="text-xs font-bold font-medium text-slate-400">Belum ada Analisis AI</p>
                              <p className="text-[10px] text-slate-500">Tekan tombol 'Ulas via AI' di atas untuk memanggil model Gemini secara real-time.</p>
                            </div>
                          )}
                        </div>
                      </div>
