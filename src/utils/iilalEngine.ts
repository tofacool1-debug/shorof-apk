...
import { DataWazan, ShighotDetail, TasrifIstilahi, TasrifLughowi } from "../types";
...
export const IilalEngine = {
  ...
  // I'ILAL DASAR
  applyIilalMadhi(fa: string, ain: string, lam: string, bina: string): string {
    const ashl = `${fa}${FATHA}${ain}${FATHA}${lam}${FATHA}`;
    if (bina === "Ajwaf") {
      // e.g. قَوَلَ -> قَالَ or بَيَعَ -> بَاعَ
      return ashl.replace(/وَ/g, "َا").replace(/يَ/g, "َا");
    }
    if (bina === "Naqis" || bina === "Lafif Maqrun" || bina === "Lafif Mafruq") {
      // e.g. دَعَوَ -> دَعَا or رَمَيَ -> رَمَى / وَقَيَ -> وَقَى
      if (lam === "و") {
        return ashl.replace(/وَ/g, "َا");
      } else {
        return ashl.replace(/يَ/g, "َى");
      }
    }
    if (bina === "Mudho'af") {
      // e.g. مَدَدَ -> مَدَّ
  ...
  applyIilalAmar(fa: string, ain: string, lam: string, bina: string, wazanMudhari: string): string {
    ...
    if (bina === "Naqis" || bina === "Lafif Maqrun" || bina === "Lafif Mafruq") {
      ...
      // If Ajwaf, e.g. قُولُ -> قُلْ (meeting of two sukuns drops the weak letter)
      if (bina === "Ajwaf") {
  ...
  buatIsimAlat(fa: string, ain: string, lam: string, bina: string, wazanMadhi: string): string {
    let word = this.replaceRoot(`مِفْعَالٌ`, fa, ain, lam);
    if (bina === "Naqis" || bina === "Lafif Maqrun" || bina === "Lafif Mafruq") {
      word = `مِ${fa}ْ${ain}َاةٌ`; // مِصْفَاةٌ
    } else if (bina === "Mudho'af") {
  ...
  buatIsimMarrah(fa: string, ain: string, lam: string, bina: string): string {
    let word = `${fa}َ${ain}ْ${lam}َةٌ`;
    if (bina === "Ajwaf") {
      word = `${fa}َوْلَةٌ`.replace("و", ain).replace("ل", lam);
    } else if (bina === "Naqis" || bina === "Lafif Maqrun" || bina === "Lafif Mafruq") {
      word = `${fa}َ${ain}ْوَةٌ`.replace("و", lam === "ي" ? "ي" : "و");
    } else if (bina === "Mudho'af") {
      word = `${fa}َ${ain}${SHADDA}َةٌ`;
    }
    return word;
  },

  buatIsimNau(fa: string, ain: string, lam: string, bina: string): string {
    let word = `${fa}ِ${ain}ْ${lam}َةٌ`;
    if (bina === "Ajwaf") {
      word = `${fa}ِيْلَةٌ`.replace("ي", ain === "و" ? "و" : "ي").replace("ل", lam);
    } else if (bina === "Naqis" || bina === "Lafif Maqrun" || bina === "Lafif Mafruq") {
      word = `${fa}ِ${ain}ْيَةٌ`.replace("ي", lam === "و" ? "و" : "ي");
    } else if (bina === "Mudho'af") {
      word = `${fa}ِ${ain}${SHADDA}َةٌ`;
    }
    return word;
  },
  ...
  buatMasdar23(fa: string, ain: string, lam: string, wazanMadhi: string, bina: string): string[] {
    const base = (wazan: string) => this.replaceRoot(wazan, fa, ain, lam);

    const list: string[] = [];
    list.push(base("فَعْلٌ"));      // 1. ضَرْبٌ
    list.push(base("فِعْلٌ"));      // 2. ضِرْبٌ
    list.push(base("فُعْلٌ"));      // 3. ضُرْبٌ
    list.push(base("فَعَلَانٌ"));   // 4. جَرَيَانٌ
    list.push(base("فَعَلَانَةٌ")); // 5. غَلَيَانَةٌ
    list.push(base("فُعَالٌ"));     // 6. كُتَابٌ
    list.push(base("فِعَالٌ"));     // 7. قِتَالٌ
    list.push(base("فَعَلَةٌ"));    // 8. سَمَاعَةٌ
    list.push(base("فَعَالَةٌ"));   // 9. فَصَاحَةٌ
    list.push(base("فِعَالَةٌ"));   // 10. زِعَامَةٌ
    list.push(base("فُعُولَةٌ"));   // 11. سُهُولَةٌ
    list.push(base("فَعِيلٌ"));     // 12. رَحِيلٌ
    list.push(base("فُعُولٌ"));     // 13. دُخُولٌ
    list.push(base("فَعَلَانٌ"));   // 14. طَيَرَانٌ
    list.push(base("فَعَلَاتٌ"));   // 15. حَسَنَاتٌ
    list.push(base("مَفْعَلَةٌ"));  // 16. مَذْهَبَةٌ
    list.push(base("فَعْلَةٌ"));    // 17. ضَرْبَةٌ Marrah
    list.push(base("فِعْلَةٌ"));    // 18. ضِرْبَةٌ Nau'
    list.push(base("تَفْعِلٌ"));    // 19. تَفْرِيقٌ
    list.push(base("تَفْعَالٌ"));   // 20. تَكْثَارٌ
    list.push(base("فُعْلَى"));     // 21. بُشْرَى
    list.push(base("فَعْلِيٌّ"));   // 22. فَقْرِيٌّ
    list.push(base("فُعْلَانٌ"));   // 23. غُفْرَانٌ

    // Apply minor adjustments for weak letters in masdars if needed
    return list.map(item => {
      let word = item;
      if (bina === "Naqis") {
        // e.g. دَعْوٌ or رَمْيٌ is fine, but double weak suffix is refined
        word = word.replace(`وٌ`, `وٌ`).replace(`يٌ`, `يٌ`);
      }
      return word;
    });
  },
  ...
  buatJamakTaksir(mufrod: string, fa: string, ain: string, lam: string, wazanMufrod: string, bina: string): string[] {
    ...
    if (cleanWazan.includes("فَاعِل")) {
      return [base("فُعَّالٌ"), base("فَعَلَةٌ"), base("فَعْلَى"), base("فُعَلَاءُ")];
    }
  ...
  tasrifIstilahiCustom(dataWazan: DataWazan): TasrifIstilahi {
    const { fa, ain, lam, wazanMadhi, wazanMudhari, masdar } = dataWazan;
    ...
    const madhi = this.applyIilalMadhi(fa, ain, lam, bina);
    const mudhari = this.applyIilalMudhari(fa, ain, lam, bina, wazanMudhari);
    ...
    const isimMusyabihatMufrod = this.buatIsimMusyabihat(fa, ain, lam, bina);
    ...
    const isimAlatMufrod = this.buatIsimAlat(fa, ain, lam, bina, wazanMadhi);
    ...
    return {
      ...
      isimAlat: this.buatShighotDetail(isimAlatMufrod, fa, ain, lam, "مِفْعَالٌ", bina),
  ...
  tasrifLughowi(tasrif: TasrifIstilahi, fa: string, ain: string, lam: string, bina: string): TasrifLughowi {
    ...
    const buildMadhi14 = (baseMadhi: string): string[] => {
      ...
      if (bina === "Mudho'af") {
        ...
        const rootSplit = `${fa}${FATHA}${ain}${SUKUN}${lam}`; // e.g. مَدَدْ
        return [
          rootMerged, // هُوَ
          rootMerged + "َا", // هُمَا m
          rootMerged.replace(`${FATHA}`, `${DAMMA}`) + "ُوا", // هُمْ
          rootMerged + "َتْ", // هِيَ
          rootMerged + "َتَا", // هُمَا f
          rootSplit + `${FATHA}ْنَ`, // هُنَّ  (مَدَدْنَ)
          rootSplit + `${FATHA}ْتَ`, // أَنْتَ
          rootSplit + `${FATHA}ْتُمَا`, // أَنْتُمَا
          rootSplit + `${FATHA}ْتُمْ`, // أَنْتُمْ
          rootSplit + `${FATHA}ْتِ`, // أَنْتِ
          rootSplit + `${FATHA}ْتُمَا`, // أَنْتُمَا
          rootSplit + `${FATHA}ْتُنَّ`, // أَنْتُنَّ
          rootSplit + `${FATHA}ْتُ`, // أَنَا
          rootSplit + `${FATHA}ْنَا`  // نَحْنُ
        ];
      }
      ...
      if (bina === "Naqis" || bina === "Lafif Maqrun" || bina === "Lafif Mafruq") {
        ...
        const rootSplit = `${fa}${FATHA}${ain}${SUKUN}${weak}`; // e.g. دَعَوْ or رَمَيْ
        return [
          baseMadhi, // هُوَ (دَعَا / رَمَى)
          baseMadhi.endsWith("َى") ? baseMadhi.slice(0, -1) + "َيَا" : baseMadhi + "وَا", // هُمَا (دَعَوَا / رَمَيَا)
          baseMadhi.slice(0, -1) + (weak === "و" ? "ُوا" : "ُوْا"), // هُمْ (دَعَوْا / رَمَوْا)
          baseMadhi.slice(0, -1) + "َتْ", // هِيَ (دَعَتْ / رَمَتْ)
          baseMadhi.slice(0, -1) + "َتَا", // هُمَا f (دَعَتَا / رَمَتَا)
          rootSplit + `${FATHA}ْنَ`, // هُنَّ (دَعَوْنَ / رَمَيْنَ)
          rootSplit + `${FATHA}ْتَ`, // أَنْتَ (دَعَوْتَ / رَمَيْتَ)
          rootSplit + `${FATHA}ْتُمَا`, // أَنْتُمَا
          rootSplit + `${FATHA}ْتُمْ`, // أَنْتُمْ
          rootSplit + `${FATHA}ْتِ`, // أَنْتِ
          rootSplit + `${FATHA}ْتُمَا`, // أَنْتُمَا
      ...
      if (bina === "Ajwaf") {
        const dammaOrKasra = (ain === "و") ? KASRA : KASRA; // Usually Kasra or Damma depending on the Bab. Let's make it intuitive:
        // قَالَ -> قُلْتَ (damma), بَاعَ -> بِعْتَ (kasra), خَافَ -> خِفْتَ (kasra)
        const leadVowel = (ain === "و" && fa === "ق") ? DAMMA : KASRA; // e.g. qul vs bi/khif
        const shortRoot = `${fa}${leadVowel}${lam}`; // e.g. قُلْ / بِعْ
        return [
          baseMadhi, // قَالَ
          baseMadhi + "ا", // قَالَا
          baseMadhi.replace(/َا/g, "ُ") + "وا", // قَالُوا
          baseMadhi + "تْ", // قَالَتْ
          baseMadhi + "تَا", // قَالَتَا
          shortRoot + `${SUKUN}ْنَ`, // قُلْنَ / بِعْنَ
          shortRoot + `${SUKUN}ْتَ`, // قُلْتَ / بِعْتَ
      ...
      const stem = `${fa}${FATHA}${ain}${FATHA}${lam}`; // e.g. نَصَرَ (without final fatha)
      return [
        stem + FATHA, // هُوَ
        stem + "َا", // هُمَا m
        stem + DAMMA + "وا", // هُمْ
    ...
    const buildMudhari14 = (baseMudhari: string): string[] => {
      ...
      if (bina === "Mudho'af") {
        ...
        const aPrefix = "أَ";
        const nPrefix = "نَ";
        const stem = `${fa}${DAMMA}${ain}${SHADDA}`; // e.g. مُدّ
        return [
          ...
          prefix + `${fa}${DAMMA}${ain}${SUKUN}${lam}${FATHA}ْنَ`, // هُنَّ (يَمْدُدْنَ)
          ...
          tPrefix + `${fa}${DAMMA}${ain}${SUKUN}${lam}${FATHA}ْنَ`, // أَنْتُنَّ (تَمْدُدْنَ)
      ...
      if (bina === "Naqis" || bina === "Lafif Maqrun" || bina === "Lafif Mafruq") {
        ...
        const stemBody = `${fa}${SUKUN}${ain}`; // e.g. دْع or رْم or رْض
    ...
    const buildAmar6 = (baseAmar: string): string[] => {
      ...
      if (bina === "Naqis" || bina === "Lafif Maqrun" || bina === "Lafif Mafruq") {
        // e.g. اُدْعُ (Anta). Dual is: اُدْعُوَا, Plural is: اُدْعُوا, Fem-Singular is: اُدْعِي, Fem-Plural is: اُدْعُنَ
        const stem = baseAmar; // اُدْعُ / اِرْمِ
        const weak = lam === "و" ? "و" : "ي";
        return [
          stem, // أَنْتَ
          stem + weak + "َا", // أَنْتُمَا
          stem + weak + "ُوا", // أَنْتُمْ
          stem + (weak === "و" ? "ِي" : "ِي"), // أَنْتِ
          stem + weak + "َا", // أَنْتُمَا f
          stem + weak + "ْنَ", // أَنْتُنَّ
        ];
      }
      if (bina === "Mudho'af") {
        // e.g. مُدَّ
        // 6 forms are: مُدَّ, مُدَّا, مُدُّوا, مُدِّي, مُدَّا, اُمْدُدْنَةٌ -> اُمْدُدْنَ
        const stem = baseAmar.slice(0, -1); // مُدّ
        const splitPrefix = `اُ${fa}${SUKUN}${ain}${DAMMA}${lam}`; // e.g. اُمْدُدْ
        return [
          ...
          splitPrefix + SUKUN + "ْنَ" // أَنْتُنَّ (اُمْدُدْنَ)
        ];
      }
      if (bina === "Ajwaf") {
        // e.g. قُلْ
        // Anta (قُلْ), Antuma (قُولَا), Antum (قُولُوا), Anti (قُولِي), Antuma (قُولَا), Antunna (قُلْنَ)
        const longVowel = ain === "و" ? "ُو" : "ِي";
        const longStem = baseAmar.replace(lam + SUKUN, "") + longVowel + lam; // e.g. قُول
        return [
          baseAmar, // أَنْتَ (قُلْ)
          longStem + FATHA + "ا", // أَنْتُمَا (قُولَا)
          longStem + DAMMA + "وا", // أَنْتُمْ (قُولُوا)
          longStem + KASRA + "ي", // أَنْتِ (قُولِي)
          longStem + FATHA + "ا", // أَنْتُمَا f
          baseAmar + "ْنَ" // أَنْتُنَّ (قُلْنَ)
        ];
      }
    ...
    const makeMajzum = (verb: string, index: number): string => {
      ...
      if (bina === "Ajwaf") {
        if (index === 0 || index === 3 || index === 6) {
          if (verb.includes("ُولُ")) {
            return verb.replace("ُولُ", "ُلْ");
          }
          if (verb.includes("ِيعُ")) {
            return verb.replace("ِيعُ", "ِعْ");
          }
          if (verb.includes("َافُ")) {
            return verb.replace("َافُ", "َفْ");
          }
          return verb.replace("ُو", "ُ").replace("ِي", "ِ").replace("َا", "َ").replace(new RegExp(`${DAMMA}$`), SUKUN);
        }
      }
      if (bina === "Mudho'af") {
        if (index === 0 || index === 3 || index === 6) {
    ...
    const buildAmar12 = (baseAmar: string): string[] => {
      ...
      const amrGhoib = mudhari14.slice(0, 6).map((verb, idx) => {
        return LI_PREFIX + makeMajzum(verb, idx);
      });
      const amrMukhotob = buildAmar6(baseAmar);
