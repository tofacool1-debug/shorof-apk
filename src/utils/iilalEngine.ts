...
export const IilalEngine = {
  ...
  buatMasdar23(fa: string, ain: string, lam: string, wazanMadhi: string, bina: string): string[] {
    ...
    list.push(base("فَعَلَةٌ"));    // 8. سَمَاعَةٌ / رَحْمَةٌ
    ...
    list.push(base("فِعْلَانٌ"));   // 14. حِرْمَانٌ / تِجْرَانٌ
    ...
    list.push(base("تَفْعِلٌ"));    // 19. تَفْرِيقٌ / تَفْعِيلٌ
    ...
    // Apply rigorous I'lal morphological and phonological adjustments for all binas in all 23 masdar forms
    return list.map(item => {
      let word = item;

      // 1. Bina' Mudho'af: Merge identical sakin 'ain and lam (Assimilation/Idgham)
      if (bina === "Mudho'af") {
        // Transfer vowel of geminated letter to previous sakin letter (e.g., مَمْدَدٌ -> مَمَدٌّ)
        const regexFathaTransfer = new RegExp(`${fa}${SUKUN}${ain}${FATHA}${lam}`, "g");
        word = word.replace(regexFathaTransfer, `${fa}${FATHA}${ain}${SHADDA}`);
        const regexKasraTransfer = new RegExp(`${fa}${SUKUN}${ain}${KASRA}${lam}`, "g");
        word = word.replace(regexKasraTransfer, `${fa}${KASRA}${ain}${SHADDA}`);
        const regexDammaTransfer = new RegExp(`${fa}${SUKUN}${ain}${DAMMA}${lam}`, "g");
        word = word.replace(regexDammaTransfer, `${fa}${DAMMA}${ain}${SHADDA}`);

        // e.g. مَدْدٌ -> مَدٌّ, مَدْدَةٌ -> مَدَّةٌ
        const regexSukun = new RegExp(`${ain}${SUKUN}${lam}`, "g");
        word = word.replace(regexSukun, `${ain}${SHADDA}`);
        const regexDirect = new RegExp(`${ain}${lam}`, "g");
        word = word.replace(regexDirect, `${ain}${SHADDA}`);
        // Handle double letters before Ta Marbutah specifically: مَدْدَة -> مَدّة
        const regexTa = new RegExp(`${ain}${SUKUN}${lam}َة`, "g");
        word = word.replace(regexTa, `${ain}${SHADDA}َة`);
      }

      // 2. Bina' Mitsal & Universal rule for sound assimilation:
      // If وْ is preceded by Kasra (e.g. in مِوْصَالٌ), the weak Waw assimilates into Ya -> مِيعَادٌ
      word = word.replace(new RegExp(`${KASRA}و${SUKUN}`, "g"), `ِي`);
      word = word.replace(new RegExp(`${DAMMA}ي${SUKUN}`, "g"), `ُو`);

      // 3. Bina' Ajwaf:
      if (bina === "Ajwaf") {
        // Kasra + weak + Sukun -> ِي (e.g. قِوْلٌ -> قِيلٌ)
        word = word.replace(new RegExp(`${KASRA}[وي]${SUKUN}`, "g"), `ِي`);
        // Damma + weak + Sukun -> ُو (e.g. بُيْضٌ -> بُوضٌ)
        word = word.replace(new RegExp(`${DAMMA}[وي]${SUKUN}`, "g"), `ُو`);
        // Kasra + و + Alif -> ِيَا (e.g. قِوَامٌ -> قِيَامٌ)
        word = word.replace(new RegExp(`${KASRA}وَ`, "g"), `ِيَا`);
        word = word.replace(new RegExp(`${KASRA}وَا`, "g"), `ِيَا`);

        // Transfer vowel of weak letter to preceding sukun letter:
        // - SUKUN + weak + FATHA -> FATHA + ا (e.g. مَقْوَلَةٌ -> مَقَالَةٌ)
        word = word.replace(new RegExp(`${SUKUN}[وي]${FATHA}`, "g"), `${FATHA}ا`);
        // - SUKUN + weak + DAMMA -> DAMMA + و (e.g. يَقْوُلُ -> يَقُولُ)
        word = word.replace(new RegExp(`${SUKUN}[وي]${DAMMA}`, "g"), `${DAMMA}و`);
        // - SUKUN + weak + KASRA -> KASRA + ي (e.g. يَبْيِعُ -> يَبِيعُ)
        word = word.replace(new RegExp(`${SUKUN}[وي]${KASRA}`, "g"), `${KASRA}ي`);
      }

      // 4. Bina' Naqis & Bina' Lafif (Maqrun & Mafruq):
      if (bina === "Naqis" || bina === "Lafif Maqrun" || bina === "Lafif Mafruq") {
        // Weak letter preceded by Alif at the end of the word changes to Hamzah (e.g., دُعَاوٌ -> دُعَاءٌ, بِنَايٌ -> بِنَاءٌ)
        word = word.replace(/ا[وي]([ًٌٍ])/g, "اء$1");
        word = word.replace(/ا[وي]َة/g, "اءَة");

        // Merge double weak vowels at word endings (e.g., رَمِيْوٌ -> رَمِيٌّ, دُعُوْوٌ -> دُعُوٌّ)
        word = word.replace(/ِي[وي]([ًٌٍ])/g, "ِيّ$1");
        word = word.replace(/ُو[وي]([ًٌٍ])/g, "ُوّ$1");
        word = word.replace(/ِي[وي]َة/g, "ِيَّة");
        word = word.replace(/ُو[وي]َة/g, "ُوَّة");

        // Fix final suffix vowels if they end in standard lam weak letters
        if (word.endsWith(`فُعْلَى`)) {
          word = word.replace(new RegExp(`${lam}$`), `َى`);
        }
      }

      // 5. Bina' Mahmuz:
      // Prevent consecutive meeting of double hamzahs or awkward starting/intermediate configurations
      word = word.replace(/أَأْ/g, "آ")
                 .replace(/أَءْ/g, "آ")
                 .replace(/إِأْ/g, "إِي")
                 .replace(/إِئْ/g, "إِي")
                 .replace(/إِء/g, "إِي");
